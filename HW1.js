...
/*Є список ролей - const roles = ["user", "editor", "admin"]; Перевір чи роль "admin" присутня. 
Обгорни це в функцію, яка повертає "Access granted" якщо є “admin”, і "Access denied" якщо ні.*/
...
/*Є масив HTTP кодів. Порахуй скільки кодів означають помилку (>= 400). 
const codes = [200, 301, 404, 500, 201]; Написати функцію яка повертає кількість таких кодів.*/
...
/*Напиши функції які повертають максимальне і мінімальне значення з цього списку:
const responseTimes = [350, 120, 500, 200];*/
...
/*Є масив статусів тестів. Перевір що всі тести пройшли. const testResults = ["passed", "passed", "failed"]; 
поверни true якщо всі пройшли і false якщо ні.*/
...
/*В тебе є двомірний масив (список в списку). Пройдись по зовнішньому списку і перевір чи кожен внутрішній список є відсортованим. 
Поверни список з true i false, де відповідно true якщо список відсортований і false якщо ні.
const arrays = [
  [1, 2, 3, 4],      
  [5, 3, 8],        
  [10, 20, 30],     
...
/*Є масив статусів тестів. const testResults = ["passed", "passed", "failed"]; Напиши функцію з звичайним loop і 
if’ом, де ти ітеруєш по цьому списку і перевіряєш чи значення passed. Якщо ні - виконання функції автоматично завершується.
